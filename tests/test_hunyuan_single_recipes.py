...
def load_module(
    ...
    old = {}
    for key, value in (stubs or {}).items():
        old[key] = __import__("sys").modules.get(key)
        __import__("sys").modules[key] = value
    try:
...
def video_module():
    ...
    utils.export_to_video = object
    pil = types.ModuleType("PIL")
    pil.Image = object
    return load_module(
        ROOT / "adapters/video/hunyuan-video-15-native/run.py",
        "hunyuan_video_adapter",
        {"torch": torch, "diffusers": diffusers, "diffusers.utils": utils, "PIL": pil},
    )
...
class HunyuanSingleRecipeAuthorityTests(unittest.TestCase):
    ...
    def test_current_upstream_revisions_and_tencent_license_acceptance_are_explicit(
        ...
        for slug, revision in expected.items():
            self.assertEqual(
    ...
    def test_offloaded_hunyuan_video_envelope_fits_healthy_spark(self) -> None:
        for slug in RECIPE_SLUGS[:3]:
            memory = load(ROOT / "recipes" / f"{slug}.json")["topology"]["roles"][0][
                "resources"
            ]["memory"]
...
class HunyuanMediaAdapterTests(unittest.TestCase):
    def test_video_probe_requires_exact_mp4_contract(self) -> None:
        ...
        valid = {
            ...
            "format": {"format_name": "mov,mp4,m4a,3gp,3g2,mj2", "duration": "5.042"},
        }
        with patch.object(
            module.subprocess,
            "run",
            return_value=types.SimpleNamespace(stdout=json.dumps(valid)),
        ):
    ...
    def test_foley_validates_invalid_wav_and_timeout_before_atomic_publish(
        self,
    ) -> None:
        adapter_path = (
        ...
        self.assertLess(
            source.index("_validate_wav(outputs[0])"), source.index("os.replace(")
        )
        self.assertIn('staging = args.output_dir / ".staging"', source)

    def test_foley_validates_wav_before_atomic_publish(self) -> None:
        path = ROOT / "adapters/audio/hunyuan-video-foley-native/pipelines/run.py"
        module = load_module(path, "hunyuan_foley_adapter")
        with tempfile.TemporaryDirectory() as directory:
            valid = Path(directory) / "valid.wav"
            with wave.open(str(valid), "wb") as audio:
                audio.setnchannels(1)
