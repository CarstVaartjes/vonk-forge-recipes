...
class Ltx25Fp8CanaryTests(unittest.TestCase):
    def test_canary_reuses_exact_model_selection_and_has_bounded_resources(
        self,
    ) -> None:
    ...
    def test_source_bundle_path_and_package_bind_current_recipe(self) -> None:
        ...
        archive, _, bundle_digest = runpy.run_path(
            str(ROOT / "tools/build-catalog-index")
        )["source_bundle"](CANARY_ADAPTER)
        self.assertEqual(
        ...
        entry = next(
            ...
            if item["source_path"] == f"recipes/{CANARY_SLUG}.json"
        )
        self.assertEqual(
