...
def test_multistage_digest_pins_are_manifested_once_and_stage_reuse_is_internal() -> (
    None
):
...
""" % (b"a" * 64, b"b" * 64, b"b" * 64)
    assert TOOL["_dockerfile_build_inputs"](content, label="Dockerfile") == [
        {
            "kind": "oci-image",
            "reference": "registry.example/base:builder@sha256:" + "a" * 64,
            "platform": "linux/arm64",
        },
        {
            "kind": "oci-image",
            "reference": "registry.example/base:runtime@sha256:" + "b" * 64,
            "platform": "linux/arm64",
        },
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (b"FROM ${BASE}\n", "digest pinned"),
        (
            b"FROM --platform=linux/amd64 registry.example/base@sha256:"
            + b"a" * 64
            + b"\n",
            "ARM64",
        ),
