...
def _request(name: str, computed: int, prompt: int, tokens: int | None = None, placeholders: int = 0):
    return _Request(
        name,
        computed,
        prompt,
...
def _apply_issue27(patch_path: Path, scheduler_path: Path) -> None:
    ...
    with patch.object(sys, "argv", [str(patch_path)]), contextlib.redirect_stdout(io.StringIO()):
        try:
...
def _load_patched_scheduler(patch_path: Path, running=(), tracked=(), config_cap: int = 1):
...
class MiaDeepSeekRecentPatchTests(unittest.TestCase):
    def test_recipe_revisions_retain_source_and_runtime_pins_with_verified_tree_hashes(self) -> None:
        expected = (
            (
        ...
        for recipe_path, dockerfile_path, version, source_pin, patch_hash in expected:
            with self.subTest(recipe=recipe_path.name):
                ...
                self.assertTrue(recipe["provenance"]["source_reference"].endswith(source_pin))
                self.assertEqual(
        ...
        self.assertIn('UPSTREAM_MERGE = "10c75477b07c2f1a361f54b7357af1019bba5fd8"', issue117)
    ...
    def test_issue27_r3_counts_running_prefills_not_the_async_tracking_set(self) -> None:
        for adapter in DEEPSEEK_ADAPTERS:
            patch_path = adapter / "patches/hotfix-dsv4-issue27-partial-prefill-concurrency.py"
    ...
    def test_issue27_r3_is_idempotent_and_refuses_an_older_gate(self) -> None:
        for adapter in DEEPSEEK_ADAPTERS:
            patch_path = adapter / "patches/hotfix-dsv4-issue27-partial-prefill-concurrency.py"
            with self.subTest(adapter=adapter.name), tempfile.TemporaryDirectory() as directory:
                path = Path(directory) / "scheduler.py"
                path.write_text(ISSUE27_FIXTURE, encoding="utf-8")
                _apply_issue27(patch_path, path)
    ...
    def test_issue55_truncation_keeps_empty_lists_serializable_and_upgrades_old_patch(self) -> None:
            ...
            with self.subTest(adapter=adapter.name):
                ...
                for streaming in (True, False):
                    for reason, calls, expected in cases:
                        ...
                        exec(hotfix["HELPER_NEW"], namespace)
                        source = (
                ...
                for old_stream, old_chat in ((True, True), (True, False), (False, True)):
                    with tempfile.TemporaryDirectory() as directory:
                        ...
                        source = hotfix["HELPER_NEW"] + hotfix["STREAMING_OLD"] + "\n" + hotfix["NOSTREAM_OLD"]
                        source = source.replace(
                            hotfix["STREAMING_OLD"],
                            hotfix["STREAMING_PREVIOUS"] if old_stream else hotfix["STREAMING_NEW"],
                        )
                        source = source.replace(
                            hotfix["NOSTREAM_OLD"],
                            hotfix["NOSTREAM_PREVIOUS"] if old_chat else hotfix["NOSTREAM_NEW"],
                        )
                        ...
                        with patch.object(sys, "argv", [str(patch_path), str(root)]):
                            ...
                            self.assertEqual(hotfix["main"](), 0)
                            self.assertEqual(serving.read_text(encoding="utf-8"), upgraded)
