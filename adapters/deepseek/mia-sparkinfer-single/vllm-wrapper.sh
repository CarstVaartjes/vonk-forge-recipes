...
die() {
  printf 'mia-sparkinfer-single: %s\n' "$*" >&2
  exit 2
}
...
[[ ${served_model_name} == deepseek-v4-flash-0731 ]] || die "unexpected served model name"
[[ ${host} == 0.0.0.0 ]] || die "the endpoint must bind to 0.0.0.0"
[[ ${port} == 8000 ]] || die "the catalog interface requires port 8000"
[[ ${max_model_len} == 384000 ]] || die "the upstream-tested model limit is 384000"
[[ ${max_num_seqs} == 1 ]] || die "the upstream-tested profile is single-request"
[[ ${max_num_batched_tokens} == 8224 ]] || die "the validated batch-token limit is 8224"
[[ ${max_cudagraph_capture_size} == 24 ]] || die "the upstream-tested CUDA graph width is 24"
[[ ${gpu_memory_utilization} == 0.94 ]] || die "the upstream-tested memory utilization is 0.94"
...
export CUDAGRAPH_CAPTURE_SIZES=6,12,24
export DEFAULT_CHAT_TEMPLATE_KWARGS_EFFORT=max
export DEFAULT_CHAT_TEMPLATE_KWARGS_THINKING=true
export DCP_SIZE=1
...
export VLLM_DSV4_PADDED_NVFP4=0
...
export EXTRA_VLLM_ARGS="--long-prefill-token-threshold 1024"
