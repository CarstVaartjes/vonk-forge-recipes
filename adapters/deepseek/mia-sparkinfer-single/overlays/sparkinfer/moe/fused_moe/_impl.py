...
_DYNAMIC_WORK_SOURCE_ENV = "SPARKINFER_DYNAMIC_WORK_SOURCE"
...
_DYNAMIC_W4A8_REPACKED_ENV = "SPARKINFER_DYNAMIC_W4A8_REPACKED"
_DYNAMIC_W4A8_SHARE_INPUT_ENV = "SPARKINFER_DYNAMIC_W4A8_SHARE_INPUT"
_DYNAMIC_W4A8_MATERIALIZED_ENV = "SPARKINFER_DYNAMIC_W4A8_MATERIALIZED"
_W4A8_CONVERT_SCRATCH_MB_ENV = "SPARKINFER_W4A8_CONVERT_SCRATCH_MB"
...
class TPMoEWorkspace:
    """Reusable scratch buffers for one `sparkinfer_moe_fp4` shape family."""
...
@dataclass(frozen=True, kw_only=True)
class SPARKINFERFP4ExpertWeights:
...
def _prepared_payload_for_runtime(
    ...
    *,
    ...
    if not isinstance(experts, SPARKINFERFP4ExpertWeights):
        raise TypeError(
            "runtime MoE launches require SPARKINFERFP4ExpertWeights from "
...
class TPMoEFP4Binding:
    ...
    def run(self) -> torch.Tensor:
        return sparkinfer_moe_fp4(binding=self)
...
class TPMoERouteBinding:
    ...
    def run(self) -> SPARKINFERTopKRouting:
        return sparkinfer_route_experts_fast(binding=self)
...
class TPMoESparseFP4Binding:
    ...
    routing: SPARKINFERTopKRouting | None = None
    top_k: int | None = None
    gate_weight: torch.Tensor | None = None
    gate_bias: torch.Tensor | None = None
    router_logits: torch.Tensor | None = None
    ...
    def run(self) -> torch.Tensor | tuple[torch.Tensor, SPARKINFERTopKRouting]:
        return sparkinfer_sparse_moe_fp4(binding=self)
...
def _dynamic_deterministic_output_enabled(
    *, quant_mode: str, device: torch.device
    ...
    if _normalize_quant_mode(quant_mode) == "w4a16":
        return False
    return _env_flag("SPARKINFER_DYNAMIC_DETERMINISTIC_OUTPUT", default=False)
...
def _select_dynamic_tile_mn(
    ...
    *,
    ...
    SPARKINFER_DYNAMIC_TILE_MN / programmatic override wins (for benchmarking).
    """
    quant_mode = _normalize_quant_mode(quant_mode)
    ovr = _dynamic_tile_mn_override()
    if ovr is not None:
...
@contextmanager
def sparkinfer_moe_dispatch_context(stage: str | None):
...
_FAST_MATH_DEFAULT = _env_flag("SPARKINFER_FAST_MATH", default=True)
...
def _get_micro_dynamic_cutover_pairs(quant_mode: str = "nvfp4") -> int:
    ...
    if cached is None:
        cutover = os.environ.get("SPARKINFER_MICRO_DYNAMIC_CUTOVER_PAIRS")
        if cutover is None:
...
def _dynamic_multicta_enabled() -> bool:
    ...
    if _DYNAMIC_MULTICTA_CACHE is None:
        multicta_env = _first_env(
            "SPARKINFER_DYNAMIC_ENABLE_MULTICTA",
            "SPARKINFER_LEVEL10_ENABLE_MULTICTA",
        )
...
def _dynamic_down_scale_enabled() -> bool:
    ...
    if _DYNAMIC_DOWN_SCALE_CACHE is None:
        _DYNAMIC_DOWN_SCALE_CACHE = _env_flag(
            "SPARKINFER_ENABLE_DYNAMIC_DOWN_SCALE", default=False
        )
...
def _prepare_modelopt_nvfp4_runtime_alphas(
    ...
    *,
    ...
    scales in ``a*_gscale``. Keep that conversion in SPARKINFER preparation so
...
def plan_sparkinfer_fp4_moe_weights(
    *,
...
def _prewarm_w4a16_planned_launches(
    ...
    *,
        ...
        for token_count in token_counts:
            ...
            if is_capturing:
                if _SPARKINFER_TIMING:
                    ...
                    if total_ms >= _SPARKINFER_TIMING_THRESHOLD_MS:
                        logger.warning(
                            "sparkinfer_w4a16_prewarm timing tokens=%d capturing=%s "
    ...
    if _SPARKINFER_TIMING:
        ...
        if total_ms >= _SPARKINFER_TIMING_THRESHOLD_MS:
            logger.warning(
                "sparkinfer_w4a16_prewarm_total timing counts=%s capturing=%s total=%.3fms",
                token_counts,
                is_capturing,
                total_ms,
            )
...
def materialize_tp_moe_arena_workspaces(
    ...
    *,
    ...
    t_selected = time.perf_counter() if _SPARKINFER_TIMING else 0.0
    materialize_ms = 0.0
    prewarm_ms = 0.0
    for key, (plan, core_plan, required_nbytes) in selected.items():
        ...
        t_materialize0 = time.perf_counter() if _SPARKINFER_TIMING else 0.0
        ...
        if quant_mode == "w4a16":
            ...
            t_prewarm0 = time.perf_counter() if _SPARKINFER_TIMING else 0.0
    ...
    if _SPARKINFER_TIMING:
        ...
        if total_ms >= _SPARKINFER_TIMING_THRESHOLD_MS:
            logger.warning(
                "sparkinfer_tp_moe_materialize timing max_tokens=%d counts=%s "
...
def build_tp_moe_sparse_fp4_binding(
    *,
    ...
    if not isinstance(experts, SPARKINFERFP4ExpertWeights):
        raise TypeError("experts must be a SPARKINFERFP4ExpertWeights")
    swiglu_limit, swiglu_alpha, swiglu_beta = _normalize_swiglu_params(
        experts.activation,
        swiglu_limit,
        swiglu_alpha,
...
def _get_impl_mac(impl: str, *, routed_rows: int | None = None) -> int:
    ...
    override_name = f"SPARKINFER_{impl.upper()}_MAX_ACTIVE_CLUSTERS"
    if impl.startswith("dynamic"):
        mac_override = _first_env(
            override_name,
            "SPARKINFER_DYNAMIC_MAX_ACTIVE_CLUSTERS",
            "SPARKINFER_LEVEL10_MAX_ACTIVE_CLUSTERS",
        )
    else:
...
def _get_micro_kernel(
    ...
    *,
    ...
    compile_kwargs = {}
    compile_options = os.environ.get("SPARKINFER_DIRECT_CUTE_OPTIONS", "")
    if compile_options:
...
def _get_dynamic_kernel(
    ...
    *,
    ...
    # inside one SF atom (env override for dev: SPARKINFER_DYNAMIC_SWAP_AB=0/1).
    swap_ab = bool(
        quant_mode == "nvfp4"
...
@torch.library.custom_op(
    "sparkinfer::tp_moe_dynamic_launch",
...
def _tiny_decode_enabled() -> bool:
    # Default on; SPARKINFER_W4A8_TINY_DECODE=0 is the kill switch.
    return os.environ.get("SPARKINFER_W4A8_TINY_DECODE", "1") != "0"
...
@torch.library.custom_op(
    "sparkinfer::tp_moe_tiny_decode_launch",
    mutates_args="unknown",
)
...
@torch.library.custom_op(
    "sparkinfer::tp_moe_compact_micro_launch",
    mutates_args="unknown",
)
...
def _launch_micro(
    *,
    ...
    if quant_mode == "w4a8_mx":
        ...
        torch.ops.sparkinfer.tp_moe_tiny_decode_launch(
            workspace.barrier_count,
            workspace.barrier_epoch,
...
def sparkinfer_moe_fp4(*, binding: TPMoEFP4Binding) -> torch.Tensor:
    ...
    if not isinstance(experts, SPARKINFERFP4ExpertWeights):
        raise TypeError("binding.experts must be a SPARKINFERFP4ExpertWeights")
    a1_gscale = experts.a1_gscale
    ...
    if apply_router_weight_on_input and quant_mode != "w4a16":
        raise NotImplementedError(
            "apply_router_weight_on_input is not implemented in sparkinfer_moe_fp4"
        )
...
def _validate_sparse_routing(
    hidden_states: torch.Tensor, routing: SPARKINFERTopKRouting
) -> None:
...
def sparkinfer_sparse_moe_fp4(
    *, binding: TPMoESparseFP4Binding
) -> torch.Tensor | tuple[torch.Tensor, SPARKINFERTopKRouting]:
    ...
    if routing is not None:
        ...
        selected = sparkinfer_route_experts_fast(
    ...
    routed_output = sparkinfer_moe_fp4(binding=moe_binding)
