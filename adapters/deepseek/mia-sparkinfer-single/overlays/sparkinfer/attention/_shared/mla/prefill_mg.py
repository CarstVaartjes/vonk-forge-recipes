...
# Defaults for the optional dynamic MLIR operands. Hoisted to module-level
# singletons so the ``@cute.jit`` signatures keep value defaults without
# calling ``Int32``/``Int64`` in the argument list.
_EXTRA_PAGE_BLOCK_SIZE_DEFAULT = Int32(1)
_EXTRA_STRIDE_KV_BLOCK_DEFAULT = Int64(0)
_USE_EXTRA_SECTION_DEFAULT = Int32(0)
_KV_SC_BASE_ADDR_DEFAULT = Int32(0)
...
def _cache_block_stride_bytes(
    ...
    *,
    ...
    if record_bytes is not None and (is_glm or int(record_bytes) != 584):
...
def s2_qk_rope_regs_mg_glm(
    ...
    *,
    ...
    extra_page_block_size: Int32 = _EXTRA_PAGE_BLOCK_SIZE_DEFAULT,
    extra_stride_kv_block: Int64 = _EXTRA_STRIDE_KV_BLOCK_DEFAULT,
    use_extra_section: Int32 = _USE_EXTRA_SECTION_DEFAULT,
):
...
def s0_load_q_bf16_to_smem_mg(
    ...
    *,
    ...
    bar_kw = {"barrier_id": barrier_id, "number_of_threads": num_threads}
    ...
    while i < Int32(n_hg * hpb * d_nope):
        ...
        if cutlass.const_expr(n_hg == 2) and g != Int32(0):
...
def _nvfp4_pair_bfloat2_mg(
    ...
    *,
    ...
    kv_sc_base_addr: Int32 = _KV_SC_BASE_ADDR_DEFAULT,
) -> Uint32:
...
def s4_online_softmax_mg2(
    ...
    *,
    ...
    if cutlass.const_expr(two) and fmin_f32(alpha10, alpha11) < Float32(1.0):
        for vc in cutlass.range_constexpr(n_acc_tiles):
...
def s4_finalize_row_sum_mg2(
    ...
    *,
    ...
    if tid_flat < Int32(reduce_heads):
        ...
        if cutlass.const_expr(two) and group != Int32(0):
...
class UnifiedPrefillMGKernel:
    ...
    def _body(
        ...
        *,
        ...
        # XV-NoPE consumes it.
        if cutlass.const_expr((is_glm or is_nvfp4) and not t.latent_scale_per_token):
            kv_sc_addr = Int32(0)
        else:
        ...
        if tid < Int32(n_hg * t.hpb):
            ...
            if cutlass.const_expr(n_hg == 2) and m_group != Int32(0):
        ...
        if is_io:
            ...
            for lc in cutlass.range(loop_tiles, unroll=1):
                ...
                if cutlass.const_expr(has_extra) and ci >= num_main_tiles:
                ...
                if cutlass.const_expr(is_nvfp4):
                    ...
                    if cutlass.const_expr(reload_bf16_qrope) and ci > Int32(0):
                    ...
                    if cutlass.const_expr(has_extra) and ci >= num_main_tiles:
                        ...
                        rope_base_ptr = get_ptr_as_int64(extra_kv_cache_u8, rope_base)
...
def _sparse_mla_prefill_mg_flat_launch(
    ...
    *,
    ...
    pack_hilo_rows = (
        ...
        and int(mg_n_hg) == 1
        and valid_hpb == 8
        and os.environ.get("SPARKINFER_MLA_SM120_PREFILL_PACK_HILO_ROWS", "1")
...
@torch.library.custom_op(
    "sparkinfer::sparse_mla_sm120_prefill_mg",
...
@torch.library.custom_op(
    "sparkinfer::sparse_mla_sm120_prefill_mg_dual",
    mutates_args=("output", "lse_out"),
)
    ...
    *,
    ...
    if scale_format == ScaleFormat.NVFP4_E4M3:
        if kv_cache.ndim >= 3:
            ...
            raise ValueError(
                "flat NVFP4 sparse MLA pages require an explicit fp8_rope value"
            )
    ...
    if full_head_tile and active_heads == total_heads and head_offset == 0:
        torch.ops.sparkinfer.sparse_mla_sm120_prefill_mg(
            q,
            _cache_base_tensor(kv_cache),
            topk_indices,
            topk_length,
