...
die() {
  printf 'sparkinfer-target-only-single: %s\n' "$*" >&2
  exit 2
}
...
[[ ${max_model_len} == 262144 ]] || die "the target-only canary limit is 262144"
[[ ${max_num_seqs} == 4 ]] || die "the target-only canary uses four scheduling slots"
[[ ${max_num_batched_tokens} == 8192 ]] || die "the target-only canary batch-token limit is 8192"
[[ ${max_cudagraph_capture_size} == 4 ]] || die "the target-only canary CUDA graph width is four"
[[ ${gpu_memory_utilization} == 0.95 ]] || die "the target-only canary memory utilization is 0.95"
[[ ${kv_cache_dtype} == nvfp4_ds_mla ]] || die "the proven padded-FP8 sparse-MLA selector is required"
[[ ${prefix_cache} == 1 ]] || die "the target-only canary requires prefix caching"
...
readonly cache_dir=${state_root}/cache-target-only
...
unset SPEC_MODEL_PATH DSPARK_TOKENS DSPARK_CAPACITY
unset DSPARK_DYNAMIC_DRAFT_DEPTH DSPARK_DYNAMIC_DRAFT_DEPTH_WINDOW
unset DSPARK_DRAFT_ATTENTION_BACKEND DRAFT_SAMPLE_METHOD
unset VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS VLLM_USE_B12X_WO_PROJECTION

export ALLREDUCE_MODE=nccl
export BACKEND=b12x-a8
export CUDAGRAPH_CAPTURE_SIZES=1,2,4
export DCP_SIZE=1
