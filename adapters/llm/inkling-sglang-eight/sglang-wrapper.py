...
def main() -> None:
    ...
    if arguments[:2] != ["--model-path", "/models"]:
        raise SystemExit(
            "this adapter only serves the immutable /models Inkling snapshot"
        )
    ...
    if nnodes != "8" or not node_rank.isdigit() or not 0 <= int(node_rank) < 8:
        raise SystemExit(
            "the flagship Inkling topology requires exactly eight Spark ranks"
        )
    ...
    if any(not os.environ.get(name) for name in required):
        raise SystemExit(
            "distributed Inkling requires complete rendezvous and RoCE settings"
        )
    try:
        ...
        raise SystemExit(
            "distributed Inkling received an invalid rendezvous address"
        ) from None
    expected_dist_init = (
        f"{os.environ['VONK_MASTER_ADDR']}:{os.environ['VONK_MASTER_PORT']}"
    )
    if dist_init == "VONK_MASTER_ADDR:VONK_MASTER_PORT":
        ...
        raise SystemExit(
            "SGLang dist-init address does not match Vonk placement authority"
        )
