"""Offline LTX 2.5 distilled synchronized audio-video adapter."""
...
DEFAULT_PROFILE = "bf16-model-offload"
ALLOWED_REQUEST_KEYS = frozenset({"profile", "seed"})
EXPECTED_SHARDS = {
...
def _load_pipeline(profile: str):
    ...
    # On DGX Spark offload controls CUDA residency, not physical unified-memory use.
    if profile.endswith("sequential-offload"):
