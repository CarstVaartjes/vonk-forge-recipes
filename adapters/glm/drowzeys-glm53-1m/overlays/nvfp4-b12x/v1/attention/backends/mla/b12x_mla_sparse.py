...
def _is_glm_moe_dsa_model() -> bool:
    ...
    result = model_type == "deepseek_mtp" and _is_glm(target_model_type)
...
class B12xMLASparseImpl(MLAAttentionImpl[B12xMLASparseMetadata]):
    ...
    def __init__(
        ...
        **mla_args,
        ...
        # glm53 port: GLM-5.3's kpool indexer has no output_physical_slots
        # attribute and writes LOGICAL token ids; convert per call in forward_mqa.
        self._indexer_logical_ids = indexer is not None and not hasattr(
            indexer, "output_physical_slots"
        )
        if (
            indexer is not None
            and not self._indexer_logical_ids
    ...
    def forward_mqa(
        ...
        if use_ckv_gather:
            ...
            if getattr(self, "_indexer_logical_ids", False):
                # glm53 port: logical token ids -> flat physical slots
                # (block*BLOCK_SIZE+local), same contract as the SM120 backend.
                from vllm.v1.attention.backends.mla.sparse_utils import (
                    triton_convert_req_index_to_global_index as _cvt,
                )

                _rid = attn_metadata.req_id_per_token
                if _rid is None:
                    # builder only fills this on DCP/gather branches; derive
                    # from query_start_loc (same as the SM120 builder does).
