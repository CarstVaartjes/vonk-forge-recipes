...
def _pad_rope_enabled() -> bool:
    if _GLM53_PAD_ROPE in ("0", "false", "off"):
        return False
    if _GLM53_PAD_ROPE in ("1", "true", "on"):
        return True
    cap = current_platform.get_device_capability()
    return cap is not None and cap.major == 12


class PadRopeMLAWrapper(MultiHeadLatentAttentionWrapper):
    """NoPE MLA presented as rope=64 with all-zero RoPE dims (GB10/SM12x shim).

    Real projections keep their checkpoint shapes; only the inner MLAAttention (and thus
    the KV-cache spec / attention backend) sees qk_rope_head_dim=VIRTUAL_ROPE.
    """
    ...
    def forward(self, positions, hidden_states, llama_4_scaling=None):
        ...
        if self.q_lora_rank is not None:
            ...
            q_c, kv_lora = qkv_lora.split([self.q_lora_rank, self.kv_lora_rank], dim=-1)
        ...
        # ---- the shim: zero virtual RoPE dims ----
        n = q.shape[0]
...
class Glm5NextMLAAttention(nn.Module):
    def __init__(
        ...
        _wrapper_cls = (
            PadRopeMLAWrapper
            if (self.qk_rope_head_dim == 0 and _pad_rope_enabled())
            else MultiHeadLatentAttentionWrapper
        )
        self.mla_attn = _wrapper_cls(
            self.hidden_size,
