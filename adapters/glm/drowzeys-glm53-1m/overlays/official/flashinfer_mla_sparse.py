...
class FlashInferMLASparseSM120Backend(_FlashInferMLASparseBackendBase):
    ...
    def supports_combination(
        ...
        if vllm_config.model_config is not None:
            ...
            # GB10 shim: GLM-5.3 widens the index buffer by the kpool tail
            # (index_topk + kpool-1, rounded to 128). The SM120 kernel only
            # exists at width 2048, so accept any index_topk whose padded
            # buffer width is 2048 (e.g. 2044 with kpool=4).
            kpool = int(getattr(hf_text_config, "index_kpool", 1) or 1)
            width = int(index_topk) + (kpool - 1 if kpool > 1 else 0)
            width = (width + 127) // 128 * 128
            if width != 2048:
                return (
                    "FLASHINFER_MLA_SPARSE_SM120 requires padded index width 2048; "
                    f"got index_topk={index_topk} kpool={kpool} -> {width}"
                )
        return None
