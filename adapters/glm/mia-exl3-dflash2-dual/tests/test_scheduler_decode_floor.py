...
class FairTests(unittest.TestCase):
    ...
    def policy(self, **env):
        with (
            patch.dict(os.environ, {**FAIR_ENV, **env}),
    ...
    def test_single_outlier_does_not_dominate_estimate(self):
        ...
        self.p._model_cache = None
        self.assertGreater(
            self.p._est_dt(1024), 1.0
        )  # consistently slow mixed steps push 1024 over the 1.0s gate
    ...
    def test_age_and_step_budgets_are_independent(self):
        p = self.policy(
            GLM53_FAIR_PREFILL_MAX_INTERVAL_MS="10000",
            GLM53_FAIR_PREFILL_MAX_STEP_MS="100",
        )
    ...
    def test_aggregate_reservations_bound_multiple_newcomers(self):
        ...
        self.assertLessEqual(
            sum(g[1] for g in self.p._open_rec["grants"].values()), 0.4
        )
    ...
    def running_loop(self, input_budget, allocate=None):
        ...
        allocate_index = next(
            i for i, n in enumerate(loop.body) if isinstance(n, ast.With)
        )
        append_index = next(
            i
        ...
        increment_index = next(
            i
            for i in range(append_index, len(loop.body))
            if ast.unparse(loop.body[i]) == "req_index += 1"
        )
        loop.body = (
            loop.body[:allocate_index] + loop.body[append_index : increment_index + 1]
            if allocate is None
            else loop.body[: increment_index + 1]
        )
        code = compile(
    ...
    def test_prefill_cannot_preempt_incumbent_for_kv(self):
        ns = self.running_loop(
            7168, allocate=lambda r, n, **kw: [] if r is self.a else None
        )
        self.assertEqual(ns["num_scheduled_tokens"], {"A": 8})
        self.assertEqual([r.request_id for r in self.s.running], ["A", "B"])
...
def installation_tests():
    src = next(
        (
            p
            for p in [
                Path(os.environ.get("GLM53_SCHEDULER_PY_SRC", "/missing")),
                Path(
                    "/usr/local/lib/python3.12/dist-packages/vllm/v1/core/sched/scheduler.py"
                ),
                Path("/tmp/sched-live.py"),
            ]
            if p.is_file()
        ),
        None,
    ...
    clean = src.read_text()
    for marker, fn in [
        (mod.MARK_V5, mod.unpatch_v5),
        (mod.MARK_V4, mod.unpatch_v4),
        (mod.MARK_V3, mod.unpatch_v3),
        (mod.MARK_V2, mod.unpatch_v2),
    ]:
        if marker in clean:
            clean = fn(clean)
    ...
    with tempfile.TemporaryDirectory() as temp:
        for version in (0, 1, 2, 3, 4):
                ...
                if version == 4:
                    ...
                    for name in names:
                        old = (
                            mod.V3_FIN_OLD
                            if name == "FIN"
                            else getattr(mod, name + "_OLD")
                        )
                        text = mod.replace_once(
                            text, old, getattr(mod, f"V{version}_{name}_NEW"), name
                        )
            ...
            subprocess.run(
                [sys.executable, str(PATCH)], env=env, check=True, capture_output=True
            )
            installed = target.read_text()
            compile(installed, str(target), "exec")
            assert (
                mod.MARK_V5 in installed
                and mod.MARK_V4 not in installed
                and mod.MARK_V3 not in installed
                and mod.MARK_V2 not in installed
            )
            subprocess.run(
                [sys.executable, str(PATCH)], env=env, check=True, capture_output=True
            )
            ...
            drifted = installed.replace(
                ...
                "_GLM53_MIXED.finish_step_changed(self, scheduler_output)",
                1,
            )
