...
def _check_dflash_prior_groups(ns):
...
def _check_call_sites(pristine: str, text: str):
...
def _check_drafter_priority(block_pool_text: str, coordinator_text: str):
...
def _check_prior_boundary_cache_call(single_type_text: str):
...
def _check_composed_runtime_paths(
    ...
    _check_prior_boundary_cache_call(single_type_text)
    _check_drafter_priority(block_pool_text, coordinator_text)
...
def _check_composition(
    ...
    for label, order in (
        ...
        check(
            text.count("import os  # [glm53-apc-per-group]") == 1,
        ...
        _check_prior_boundary_cache_call(stm_text)
        _check_composed_runtime_paths(text, bp_text, stm_text)
        results[label] = text
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        _check_call_sites(pristine, text)
        ns = load_helpers(text)
        _check_min_exemption(ns)
        _check_routing(ns)
        _check_resolve(ns)
        _check_dflash_prior_groups(ns)
        _check_env(ns)
        _check_validator(ns)
        _check_drafter_priority(bp_text, text)
        _check_prior_boundary_cache_call(stm_text)
        _check_composition(pristine_src, bp_src, stm_src, tmp)
