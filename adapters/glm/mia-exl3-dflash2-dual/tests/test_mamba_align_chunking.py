...
PATCH = next(
    p
    for p in (
        HERE / "patch_mamba_align_chunking.py",
        HERE.parent / "overlay" / "patch_mamba_align_chunking.py",
    )
    if p.is_file()
)
SRC = Path(
...
STM_SRC = Path(
    os.environ.get(
        ...
        SRC.parent.parent / "single_type_kv_cache_manager.py",
    )
)
...
def split_function(path: Path):
    ...
    ns: dict = {}
    # Execute only the extracted function, avoiding heavyweight vLLM imports.
...
def init_statements(path: Path):
    ...
    fn = ast.FunctionDef(
        ...
        args=ast.arguments(
            posonlyargs=[],
            args=[ast.arg(arg="self"), ast.arg(arg="kv_cache_config")],
            kwonlyargs=[],
            kw_defaults=[],
            defaults=[],
        ),
    ...
    # Execute only the extracted derivation with the minimal test dependency.
    exec(  # noqa: S102
        compile(
...
def chunk_ends(
    split, sched: Sched, num_prompt: int, allowance: int, start: int = 0
) -> list[int]:
...
def part_a(src: Path) -> None:
    ...
    r2 = apply(src)
    check(
        r2.returncode == 0 and src.read_bytes() == before,
        "A2 second application is a byte-identical no-op",
    )
    if MARK not in original:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            r3 = apply(old)
            check(
                r3.returncode != 0 and old.read_text() == unsupported,
                "A4 an older decode-floor version is rejected without writing",
            )
    else:
        print(
            "  note: source already carries this overlay; A4 requires an unpatched scheduler"
        )
...
def part_b(src: Path) -> None:
    ...
    groups = [
        types.SimpleNamespace(kv_cache_spec=types.SimpleNamespace(block_size=MAMBA)),
        types.SimpleNamespace(kv_cache_spec=MambaSpec(MAMBA)),
        types.SimpleNamespace(kv_cache_spec=MambaSpec(MAMBA)),
        types.SimpleNamespace(kv_cache_spec=types.SimpleNamespace(block_size=DRAFT)),
    ]
    for eagle_ids, expect in (({6}, False), ({0, 1, 2, 3}, True), (set(), False)):
        ...
        s.kv_cache_manager = types.SimpleNamespace(
            coordinator=types.SimpleNamespace(eagle_group_ids=eagle_ids)
        )
        derive(s, types.SimpleNamespace(kv_cache_groups=groups))
        ends = chunk_ends(split, s, 30720, 7160)
        check(
            None not in ends
            and all(end % MAMBA == 0 for end in ends[:-1])
            and ((28672 not in ends) is expect),
            f"B1 target EAGLE groups {sorted(eagle_ids)} determine the last reusable checkpoint",
        )

    stock = Sched(budget=7168, block_size=MAMBA, backoff=False)
    ends = chunk_ends(split, stock, 30720, 7160)
    check(
        ends == [MAMBA * k for k in range(1, 9)] + [30720],
        "B3 stock prefill checkpoints each Mamba boundary before its partial tail",
    )
    ends = chunk_ends(
        split, Sched(budget=7168, block_size=MAMBA, backoff=True), 30720, 7160
    )
    check(
        28672 not in ends and 25088 in ends,
        "B4 with the EAGLE back-off the last checkpoint is skipped (documented full-attention drop)",
    )
    old = Sched(budget=7168, block_size=DRAFT, backoff=True)
    check(
        28672 not in chunk_ends(split, old, 30720, 7160),
        "B5 drafter-page alignment misses the 28672 checkpoint (the SD1 loss)",
    )
    ...
    check(
        None not in ends
        and all(b in ends for b in range(MAMBA, 30720, MAMBA))
        and max(e - p for p, e in zip([0, *ends], ends)) <= 1016,
        "B6 sub-block budget advances inside a block and stops at each boundary",
    )
    resumed = chunk_ends(split, stock, 30720, 7160, start=25088)
    check(
        resumed == [28672, 30720],
        "B7 a resumed prefix hit at 25088 checkpoints 28672 before the last chunk",
    )
    if "_glm53_align_prefill_limit" in src.read_text():
        ...
        check(
            None not in ends and ends[0] == 3576 and ends[1] == MAMBA,
            "B8 a per-request cap below the block (decode-floor v5) keeps nonzero sub-block progress",
        )
    else:
        print(
            "  note: no decode-floor v5 in the source; B8 (capped sub-block progress) not exercised"
        )
...
def part_c(src: Path) -> None:
    ...
    def cache(
        ...
        for index in range(num_cached_blocks, num_full_blocks):
            if not blocks[index].is_null and (
                block_mask is None or block_mask[index - num_cached_blocks]
            ):
                checkpoints.append(
                    ((index + 1) * block_size, state_positions[blocks[index].block_id])
                )
    ...
    check(
        checkpoints == [(3584, 3584)],
        "C1 mixed caps hash the state after 3584 tokens, not the prior state after 2688",
    )


def main() -> int:
    for var, path in (
        ("GLM53_SCHEDULER_PY", SRC),
        ("GLM53_SINGLE_TYPE_KV_CACHE_MANAGER_PY", STM_SRC),
    ):
        if not path.is_file():
