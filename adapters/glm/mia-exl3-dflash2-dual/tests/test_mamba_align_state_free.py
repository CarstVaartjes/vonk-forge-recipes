...
PATCH = next(
    p
    for p in (
        HERE / "patch_mamba_align_state_free.py",
        HERE.parent / "overlay" / "patch_mamba_align_state_free.py",
    )
    if p.is_file()
)
...
class Block:
    def __init__(self, block_id: int) -> None:
        self.block_id, self.ref_cnt, self.block_hash, self.is_null = (
            block_id,
            0,
            None,
            False,
...
def replay(manager_cls, spec_cls, chunk: int, in_flight: bool, tokens: int, reuse=None):
    ...
    while computed < tokens:
        ...
        mgr.get_num_blocks_to_allocate(
            "r", computed + new, [], computed, computed, computed + new
        )
...
def part_a(stm: Path, kvi: Path) -> None:
    ...
    r2 = apply(stm, kvi)
    check(
        r2.returncode == 0 and (stm.read_bytes(), kvi.read_bytes()) == before,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        drifted.write_text(
            before[0]
            .decode()
            .replace("stale_state_block_idxs", "stale_state_block_idxz", 1)
        )
        ...
        check(
            r3.returncode != 0
            and (drifted.read_bytes(), kvi_copy.read_bytes()) == drifted_before,
            "A4 an edited applied block is rejected without changing either file",
        )
...
def part_b(stm: Path, pristine_stm: Path) -> None:
    ...
    for label, chunk, in_flight, batches in (
        ...
        _, _pool_old, held_old = replay(
            original, original_spec_cls, chunk, in_flight, tokens
        )
        mgr, pool, held = replay(patched, spec_cls, chunk, in_flight, tokens)
        peak = max(h for h, _ in held)
        check(
            peak <= bound and pool.referenced() == held[-1][0],
            f"B1 [{label}] resident blocks stay within 1 + batches + spec = {bound} (peak {peak}, old peak {max(h for h, _ in held_old)})",
        )
        check(
            all(ok for _, ok in held),
            f"B2 [{label}] the state block feeding the in-flight step is never released",
        )
        if chunk > BLOCK and in_flight and pristine_available:
            check(
                max(h for h, _ in held_old) > bound + 10,
                f"B3 [{label}] unpatched manager retains superseded blocks ({max(h for h, _ in held_old)})",
            )
        mgr.free("r")
        check(
            pool.referenced() == 0,
            f"B4 [{label}] freeing a request returns every referenced block",
        )
        _, _, reused = replay(
            patched, spec_cls, chunk, in_flight, 6 * chunk, reuse=(mgr, pool)
        )
        check(
            max(h for h, _ in reused) <= bound and all(ok for _, ok in reused),
            f"B5 [{label}] reusing a freed request ID preserves bounded ownership and live state",
        )
        mgr.free("r")
        check(
            pool.referenced() == 0,
            f"B6 [{label}] the reused request releases all ownership",
        )
...
def part_c(kvi: Path) -> None:
    ...
    tree = ast.parse(src)
    cls = next(
        n for n in tree.body if isinstance(n, ast.ClassDef) and n.name == "MambaSpec"
    )
    fn = next(
        n
        for n in cls.body
    ...
    spec = types.SimpleNamespace(
        page_size_bytes=page, block_size=BLOCK, num_speculative_blocks=SPEC_BLOCKS
    )

    def cfg(mode, batches=2, max_len=262_144):
        return types.SimpleNamespace(
            cache_config=types.SimpleNamespace(mamba_cache_mode=mode),
            max_concurrent_batches=batches,
            model_config=types.SimpleNamespace(max_model_len=max_len),
        )

    usage = ns["max_memory_usage_bytes"]
    check(
        usage(spec, cfg("align", 2)) == page * 10
        and usage(spec, cfg("align", 1)) == page * 9,
        "C1 align: (1 + concurrent batches + spec) pages, i.e. 10 async / 9 sync here",
    )
    check(
        usage(spec, cfg("none")) == page * 8
        and usage(spec, cfg("all")) == page * (74 + 7),
        "C2 modes none / all unchanged",
    )
...
def main() -> int:
    for var, path in (
        ...
        ("GLM53_KV_CACHE_INTERFACE_PY", KVI_SRC),
    ):
        if not path.is_file():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if MARK in pristine.read_text():
            print(
                "  note: source already carries the overlay; B3 (unpatched growth) is skipped"
            )
        part_a(stm, kvi)
        part_b(stm, pristine)
        part_c(kvi)
