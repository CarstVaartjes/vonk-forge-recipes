...
def circular_slot_ids(
    positions: list[int], block_table_row: list[int], block_size: int, *, clamp: bool
) -> list[int]:
...
def replace_file(target: Path, source: str) -> None:
    temporary = target.with_name(f".{target.name}.glm53-kpool-tail.tmp")
    try:
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"kpool tail slot-map preflight failed: {error}") from error
